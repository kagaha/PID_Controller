...
%Step response for P-Controller for array of LTI Models.
Kp = 1;
for i = 1:3
    C(:,:,i) = pid(Kp);
    Kp = Kp + 10;
end
%Step reference with diff values of Kp
...
%Step disturbance with diff values of Kp
...
legend('Kp = 1', 'Kp = 11','Kp = 21')
%Step response for PI_Controller
%Step reference with diff values of Ki and Kp=21
Kp = 21;
Ki = 100;
for i = 1:5
    C(:,:,i) = pid(Kp,Ki);
...
%Step Disturbance with diff values of Ki and Kp=21
dist_cl = feedback(P_motor,C);
figure(4);
step(dist_cl(:,:,1), dist_cl(:,:,2), dist_cl(:,:,3), t)
ylabel('Position, \theta (radians)')
...
legend('Ki = 100',  'Ki = 300',  'Ki = 500')
%Step response for PID_Controller
%Step reference with diff values of Kd,Kp=21 and Ki=500
Kp = 21;
Ki = 500;
Kd = 0.05;
...
%Step Disturbance with diff values of Kd,Kp=21 and Ki=500
dist_cl = feedback(P_motor,C);
t = 0:0.001:0.2;
figure(6);
step(dist_cl(:,:,1), dist_cl(:,:,2), dist_cl(:,:,3), t)
ylabel('Position, \theta (radians)')
title('Step Disturbance with K_p = 21, K_i = 500 and Different values of K_d')
legend('Kd = 0.05', 'Kd = 0.15', 'Kd = 0.25')
